...
export default function Exercises() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['exercises', page, search],
    queryFn: async () => {
      ...
      const response = await api.get(`/admin/exercise-library/manage?${params}`);
  ...
  const validateForm = () => {
    ...
    // Category and muscleGroup are now optional
  ...
  const handleSaveExercise = async () => {
    ...
    try {
      const response = await api.post('/admin/exercise-library/manage', {
  ...
  const handleDeleteExercise = async (id: number) => {
    try {
      const response = await api.delete(`/admin/exercise-library/manage/${id}`);
