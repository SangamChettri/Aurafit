...
const ProfileScreen = () => {
  ...
  // Update joinedDate when user data is available
  useEffect(() => {
    if (user?.createdAt) {
      setStats(prev => ({ ...prev, joinedDate: user.createdAt }));
    }
  }, [user?.createdAt]);
      ...
      for (let i = 0; i < 30; i++) {
        if (Math.random() > 0.4) {
          mockDays.push({
            ...
            totalDuration: Math.floor(Math.random() * 60) + 30,
          });
        }
      }
  ...
  const saveBodyMetrics = async (newMetrics: BodyMetrics) => {
    await AsyncStorage.setItem('bodyMetrics', JSON.stringify(newMetrics));
    ...
    <View style={styles.container}>
      ...
      <View style={styles.tabContainer}>
        {[
          ...
          { id: 'exercises', icon: 'fitness-center', label: 'Exercises' },
...
const styles = StyleSheet.create({
  ...
  settingsItemText: { fontSize: 15, marginLeft: 12, color: '#374151' },
  settingsDivider: { height: 1, backgroundColor: '#e5e7eb', marginHorizontal: 8 },
