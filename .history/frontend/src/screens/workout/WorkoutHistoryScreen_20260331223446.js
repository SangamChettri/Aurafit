...
const WorkoutHistoryScreen = () => {
  ...
  const handleEditWorkout = (workout) => {
    navigation.navigate('WorkoutForm', { workoutId: workout._id, workout });
  };
  ...
  const confirmDeleteWorkout = async () => {
    ...
    const result = await deleteWorkout(selectedWorkout._id);
  ...
  /**
   * Render workout type icon as Ionicons
   */
  const renderWorkoutIcon = (type, size = 16, color = '#fff') => {
    return <Ionicons name={getWorkoutTypeIcon(type)} size={size} color={color} />;
  };
        ...
        <View style={styles.workoutInfo}>
          <View style={styles.workoutTitleRow}>
            <View style={styles.workoutIcon}>
              {renderWorkoutIcon(item.type, 16, '#fff')}
            </View>
              ...
              {['STRENGTH', 'CARDIO', 'MIXED', 'HIIT', 'YOGA'].map((type) => (
                ...
                >
                  <Text style={[
                    ...
                    {renderWorkoutIcon(type, 14, filterType === type ? theme.colors.primary : theme.colors.textSecondary)} {type}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
