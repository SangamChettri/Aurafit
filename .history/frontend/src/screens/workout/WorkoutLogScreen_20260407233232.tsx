...
const FALLBACK_CATEGORIES = ['Chest', 'Back', 'Shoulders', 'Biceps', 'Triceps', 'Legs', 'Core', 'Cardio'];
...
const WorkoutLogScreen = () => {
  ...
  const { templateWorkout } = (route.params || {}) as any;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (exerciseRes.status === 'fulfilled' && (exerciseRes.value as any).success) {
          const formattedExercises = (exerciseRes.value as any).data.map((ex: any) => ({
            id: ex.id.toString(),
            name: ex.name,
        ...
        if (contentRes.status === 'fulfilled' && (contentRes.value as any).success) {
          setCategories((contentRes.value as any).data.categories || FALLBACK_CATEGORIES);
        } else {
  ...
  const addExerciseToWorkout = (exercise: Exercise) => {
    ...
    const newWorkoutExercise: WorkoutExercise = {
      ...
      category: exercise.libraryRecord?.category || exercise.exercise?.category,
      equipment: exercise.libraryRecord?.equipment || exercise.exercise?.equipment,
      sets: [
        {
          id: Date.now().toString(),
          reps: 0,
    ...
    fetchPreviousWorkoutDataAndUpdate(exercise.exerciseId, newWorkoutExercise.id);
    
    setShowExerciseModal(false);
    setSearchQuery('');
    setSelectedCategory(null);
    
    addNotification(`Added ${exercise.exerciseName} to workout`, 'success');
  };

  const fetchPreviousWorkoutDataAndUpdate = async (exerciseId: string, workoutExerciseId: string) => {
    try {
  ...
  const onSubmit = async (data: WorkoutFormData) => {
    ...
    try {
      ...
      const workoutData: Partial<Workout> = {
        ...data,
        type: data.type.toLowerCase() as 'strength' | 'cardio' | 'hiit' | 'yoga' | 'pilates' | 'crossfit' | 'sports' | 'other',
  ...
  const getFilteredExercises = () => {
    ...
    if (selectedCategory) {
      filteredExercises = filteredExercises.filter((ex: Exercise) => 
        ex.libraryRecord?.category === selectedCategory || ex.exercise?.category === selectedCategory
      );
    }
    ...
    if (searchQuery) {
      filteredExercises = filteredExercises.filter((ex: Exercise) => 
        ...
        (!selectedCategory || ex.libraryRecord?.category === selectedCategory || ex.exercise?.category === selectedCategory)
      );
    }
    ...
    >
      <View style={styles.exerciseInfo}>
        ...
        <Text style={styles.exerciseDetails}>
          {item.libraryRecord?.category || item.exercise?.category || 'General'} 
          {' '}
          {item.libraryRecord?.muscleGroups?.join(', ') || item.exercise?.muscleGroups?.join(', ') || 'Full Body'}
        </Text>
        {(item.libraryRecord?.equipment || item.exercise?.equipment) && (
          <Text style={styles.exerciseEquipment}>
            Equipment: {item.libraryRecord?.equipment || item.exercise?.equipment}
          </Text>
        )}
      </View>
