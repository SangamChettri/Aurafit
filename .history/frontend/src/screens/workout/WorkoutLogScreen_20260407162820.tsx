...
const WorkoutLogScreen = () => {
  ...
  const addExerciseToWorkout = (exercise: Exercise) => {
    ...
    console.log('🏋️ Adding exercise to workout:', exercise.name);
    console.log('📊 Previous data available:', previousData);
    ...
    console.log('📝 Created new workout exercise with initial reps:', newWorkoutExercise.sets[0].reps, 'and weight:', newWorkoutExercise.sets[0].weight);
  ...
  const fetchPreviousWorkoutData = async (exerciseId: string) => {
    try {
      console.log('🔍 Fetching previous workout data for exercise:', exerciseId);
      const response = await api.get(`/workouts/exercise/${exerciseId}/previous`);
      console.log('📊 Previous workout response:', response.data);
      
      if (response.data.success && response.data.data) {
        console.log('✅ Previous workout data found:', response.data.data);
        ...
        console.log('📝 Updated previousWorkoutData state');
      } else {
        console.log('📊 No previous workout data returned');
      }
    } catch (error) {
  ...
  const renderWorkoutExercise = (workoutExercise: WorkoutExercise) => (
    <View key={workoutExercise.id} style={styles.workoutExerciseCard}>
      ...
      {(() => {
        console.log('🔍 Checking previous data for exercise:', workoutExercise.exerciseId);
        console.log('📊 Available previous data:', previousWorkoutData);
        const hasPreviousData = previousWorkoutData[workoutExercise.exerciseId];
