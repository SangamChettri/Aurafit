...
// Public exercises endpoint for mobile app (alias for exercise-library)
...
// Public exercise by ID endpoint
 ...
 * @access  Private (Admin/Super Admin)
 */
router.post('/exercise-library/manage',
  [
    body('name').notEmpty().trim().withMessage('Name is required'),
    body('category').optional(),
    body('muscleGroup').optional()
  ],
  asyncHandler(async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    try {
      ...
      await logAdminAction(req.user.id, 'CREATE', 'ExerciseLibrary', exercise.id, { name: exercise.name }, req);
 ...
 * @access  Private (Admin/Super Admin)
 */
router.put('/exercise-library/manage/:id', asyncHandler(async (req, res) => {
  const exerciseId = parseInt(req.params.id);
 ...
 * @access  Private (Admin/Super Admin)
 */
router.delete('/exercise-library/manage/:id', asyncHandler(async (req, res) => {
  const exerciseId = parseInt(req.params.id);
