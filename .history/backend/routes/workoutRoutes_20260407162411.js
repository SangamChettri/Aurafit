...
router.get('/exercise/:exerciseId/previous', asyncHandler(async (req, res) => {
  try {
    ...
    const avgReps = Math.round(completedSets.reduce((sum, set) => sum + set.reps, 0) / completedSets.length);
    const avgWeight = Math.round(completedSets.reduce((sum, set) => sum + set.weight, 0) / completedSets.length);
