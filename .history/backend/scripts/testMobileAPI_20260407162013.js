...
async function testMobileAPI() {
  ...
  try {
    ...
    const exercisesResponse = await fetch(`${baseUrl}/admin/exercises`);
    console.log(`   All Exercises: ${exercisesResponse.ok ? '✅' : '❌'} (${exercisesResponse.status})`);
    
    const searchResponse = await fetch(`${baseUrl}/admin/exercises?search=bench`);
    console.log(`   Search Exercises: ${searchResponse.ok ? '✅' : '❌'} (${searchResponse.status})`);
    
    const singleExerciseResponse = await fetch(`${baseUrl}/admin/exercises/1`);
