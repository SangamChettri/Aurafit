...
async function testExercisesAPI() {
  try {
    console.log('🔍 Testing /api/exercises endpoints...');
    
    // Test GET /exercises (without authentication)
    console.log('\n🏋️ Testing GET /exercises (public)...');
    ...
    const searchResponse = await fetch('http://localhost:5000/api/exercises?search=bench');
    ...
    const singleResponse = await fetch('http://localhost:5000/api/exercises/1');
