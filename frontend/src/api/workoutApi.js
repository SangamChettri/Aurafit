...
export const workoutApi = {
   ...
   * @param {string} workoutData.type - Workout type
  ...
  createWorkout: async (workoutData) => {
    try {
      ...
      // Backend expects workout details
      const dataToSend = {
        ...workoutData
      };

      const response = await axiosInstance.post('/workouts', dataToSend);
   ...
   * @returns {Promise<Object>} Volume history data
   */
  getVolumeHistory: async () => {
    try {
      console.log('📈 Fetching volume history');

      const response = await axiosInstance.get('/workouts/volume-history');

      if (isSuccess(response)) {
        const data = getResponseData(response);
        console.log(`📈 Fetched ${data.length} volume history records`);
        return {
          success: true,
          data: data,
        };
      ...
      return { success: false, error: 'Failed to fetch volume history' };
    } catch (error) {
      const errorInfo = handleApiError(error);
      console.error('📈 Get volume history error:', errorInfo);
      return {
        success: false,
        error: errorInfo.message,
        code: errorInfo.code,
