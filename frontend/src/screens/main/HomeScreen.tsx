...
const HomeScreen = () => {
  ...
  const [inspiration, setInspiration] = useState({ title: 'Stay Motivated', text: 'Consistency is key! Even a 15-minute workout can make a difference.' });
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [inspirationResponse, profileResponse] = await Promise.all([
          contentApi.getInspiration(),
          userApi.getUserProfile() as any
        ]);
        if (inspirationResponse.success && inspirationResponse.data) {
          setInspiration(inspirationResponse.data);
        }
        if (profileResponse.success) {
          setStats({ totalWorkouts: profileResponse.data?.stats?.totalWorkouts || 0 });
        }
      } catch (error) {
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.form}>
        ...
        <View style={styles.header}>
          <View style={styles.brandContainer}>
            <View style={styles.logoContainer}>
              <Image 
                source={require('../../../assets/aurafit_white_logo.png')} 
                style={{ height: 40, width: 180, marginLeft: -10 }} 
                resizeMode="contain" 
              />
            </View>
        ...
        <View style={styles.quickActions}>
          ...
          >
            <Icon name="fitness-center" size={32} color="#0ea5e9" />
          ...
          >
            <Icon name="trending-up" size={32} color="#0ea5e9" />
            <Text style={styles.actionText}>Track Progress</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={styles.actionCard}
            onPress={() => (navigation as any).navigate('Workouts', { screen: 'PersonalizedWorkoutsScreen' })}
          >
            <Ionicons name="barbell-outline" size={28} color="#0ea5e9" />
            <Text style={styles.actionText}>Personalized Workouts</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.stats}>
          ...
          <View style={{ alignItems: 'center', marginTop: 16 }}>
            <View style={[styles.statCard, { width: '70%', paddingVertical: 36 }]}>
              <Text style={[styles.statValue, { fontSize: 48, marginBottom: 8 }]}>{stats.totalWorkouts}</Text>
              <Text style={[styles.statLabel, { fontSize: 18 }]}>Total Workouts</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.motivationSection}>
          ...
          <View style={styles.motivationCard}>
            <View style={styles.motivationIconContainer}>
              <Icon name="psychology" size={28} color="#f59e0b" />
            </View>
            <View style={styles.motivationContent}>
              <Text style={styles.motivationTitle}>{inspiration.title}</Text>
